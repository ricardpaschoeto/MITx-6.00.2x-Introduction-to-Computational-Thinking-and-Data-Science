...
def r_squared(y, estimated):
    ...
    """
    mean = sum(y)/len(y)
    numerator = np.sum(np.subtract(y,estimated)**2)
    denominator = np.sum(np.subtract(y,mean)**2)
...
def evaluate_models_on_training(x, y, models):
    ...
    for ii in range(len(models)):
        estYvals = np.polyval(models[ii], x)
        error = r_squared(y, estYvals)
        pylab.plot(x, estYvals, color = 'red', label= 'R2 = ' + str(round(error,5)))
    pylab.legend(loc='best')
    pylab.title('Goodness of Fits')
